...
- Symlinked: `AGENTS.md`, `rules/`, `hooks.json`, the response-simplifier hook, the shared response-simplifier prompt, `orchestrator.config.toml`, custom agents under `agents/`, six Codex-authored skill entries (`agent-config-setup`, `sync-mattpocock-skills`, `codex-dynamic-workflows`, `commit-guidelines`, `root-browser-control`, `superpowers-domain-context`), and five universal bridge entries (`bro`, `domain-context`, `nix-environment-setup`, `r-dev-shell`, `visual-verification`). Codex-authored files are sourced from `dotfiles/codex/`; the response prompt is shared from `dotfiles/claude/response-simplifier.md`; the five universal bridges are sourced from `dotfiles/universal-skills/`.
- Copied, not linked: the other named profiles. `home.activation.codexPolicy` seeds them from `dotfiles/codex/profiles/*.config.toml`, then reconciles owned fields into `~/.codex/<name>.config.toml` on every activation. They remain ordinary files so their relative skill paths resolve from `~/.codex` and Codex can retain mutable profile state. Editing a runtime copied profile directly is not durable for owned fields; edit its tracked template instead and rebuild. The exact, state-free `orchestrator` profile is the exception and is already live through its symlink.
