...
- The `orchestrator` profile uses the V1 multi-agent backend with up to 15 concurrent subagent threads and three nesting levels. Do not enable `features.multi_agent_v2` for that profile while V2 ignores `agents.max_depth`.
