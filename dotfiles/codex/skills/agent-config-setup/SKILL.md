...
- Symlinked: `AGENTS.md`, `rules/`, `hooks.json`, the response-simplifier hook, the shared response-simplifier prompt, custom agents under `agents/`, seven Codex-authored skill entries (`agent-config-setup`, `sync-mattpocock-skills`, `codex-dynamic-workflows`, `commit-guidelines`, `root-browser-control`, `session-handoff`, `superpowers-domain-context`), and five universal bridge entries (`bro`, `domain-context`, `nix-environment-setup`, `r-dev-shell`, `visual-verification`). Codex-authored files are sourced from `dotfiles/codex/`; the response prompt is shared from `dotfiles/claude/response-simplifier.md`; the five universal bridges are sourced from `dotfiles/universal-skills/`.
