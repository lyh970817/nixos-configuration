...
- Symlinked: `AGENTS.md`, `rules/`, `hooks.json`, the response-simplifier hook, the shared response-simplifier prompt, custom agents under `agents/`, four Codex-authored skill entries (`agent-config-setup`, `sync-mattpocock-skills`, `commit-guidelines`, `session-handoff`), and five universal bridge entries (`bro`, `domain-context`, `nix-environment-setup`, `r-dev-shell`, `visual-verification`). Codex-authored files are sourced from `dotfiles/codex/`; the response prompt is shared from `dotfiles/claude/response-simplifier.md`; the five universal bridges are sourced from `dotfiles/universal-skills/`. The linked `hooks.json` and hook trust state remain user state, but the system requirements in `modules/programs/codex.nix` allow Codex CLI to discover only the managed response-simplifier Stop hook installed under `/etc/codex/hooks`.
- Copied, not linked: every named profile, including `orchestrator`. `home.activation.codexPolicy` seeds each 0600 ordinary file from `dotfiles/codex/profiles/*.config.toml`, then reconciles owned fields into `~/.codex/<name>.config.toml` on every activation. This lets relative skill paths resolve from `~/.codex` while Codex retains hooks and other mutable or unknown runtime state. For `orchestrator`, the authored root policy and `features.multi_agent_v2` fields are owned; hook configuration and trust remain runtime state but cannot expand the system-managed hook allowlist. Editing a copied runtime profile directly is not durable for owned fields; edit its tracked template instead and rebuild.
- `~/.codex/shared-skills` is a separate out-of-store symlink to `dotfiles/agents/skills` — a Matt Pocock-managed, curated, shared 36-skill pool used across profiles, not Codex-specific or universal. It must not be linked into Claude.
