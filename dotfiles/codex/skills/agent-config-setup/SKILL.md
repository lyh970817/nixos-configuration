...
The tracked sources of truth are `/home/andongni/.nixos-config/dotfiles/codex/` for Codex-authored resources and `/home/andongni/.nixos-config/dotfiles/universal-skills/` for the six universal skills (`bro`, `domain-context`, `lavish`, `nix-environment-setup`, `r-dev-shell`, and `visual-verification`), plus `dotfiles/agents/skills` for the separate Matt Pocock-managed shared pool. Home Manager module `home/programs/mutable-configs.nix` wires it into `~/.codex` mostly via `mkOutOfStoreSymlink` — an out-of-store symlink pointing at this repo checkout (`osConfig.portable.configDir`) instead of a store copy. **Edits to symlinked files are live immediately; no rebuild needed.**

- Symlinked: `AGENTS.md`, `rules/`, and 12 explicitly wired skill entries: 6 Codex-authored skills (`agent-config-setup`, `sync-mattpocock-skills`, `codex-dynamic-workflows`, `commit-guidelines`, `root-browser-control`, `superpowers-domain-context`) and six universal bridges (`bro`, `domain-context`, `lavish`, `nix-environment-setup`, `r-dev-shell`, `visual-verification`) whose canonical source files live in `dotfiles/universal-skills/`. The six universal bridges are not Codex-owned source.
...
- `skills/<name>` resolves to `~/.codex/skills/<name>` (from `dotfiles/codex/skills/`; the six universal bridge names resolve to canonical sources in `dotfiles/universal-skills/`).
...
1. Locate the tracked source under `dotfiles/codex/` for Codex-authored resources, `dotfiles/universal-skills/` for the six universal skills, or `dotfiles/agents/skills` for the separate Matt Pocock-managed shared pool first; edit there, never a runtime copy under `~/.codex`.
