...
- `skills/<name>` resolves to `~/.codex/skills/<name>` (from `dotfiles/codex/skills/`; the five universal bridge names resolve to canonical sources in `dotfiles/universal-skills/`).
...
1. Locate the tracked source under `dotfiles/codex/` for Codex-authored resources, `dotfiles/universal-skills/` for the five universal skills, or `dotfiles/agents/skills` for the separate Matt Pocock-managed shared pool first; edit there, never a runtime copy under `~/.codex`.
