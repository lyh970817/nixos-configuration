" Name:             trinity
" Description:      Trinity color theme (Neovim compatible)
...
" Website:          https://github.com/trinity-theme/trinity-extra-theme
...
" {{{ Reset colors and set colors_name
if &background == "light"
    set background=dark
endif
...
" {{{ Color definitions
" Format: ["color-item", "style", "foreground", "background"]
...
" {{{ RGB color mapping for termguicolors
let s:cmap = [ 
...
function! s:checkargs(arg)
    if a:arg+0 == 0 && a:arg != "0"
        return a:arg
    else
        return s:cmap[a:arg+0]
    endif
endfunction
" }}}

" {{{ Apply color scheme
" Set colors for both terminal (cterm) and GUI (gui/termguicolors)
for s:col in s:colors256
    let fg_term = s:col[2]
    let bg_term = s:col[3]
    let fg_gui = s:checkargs(s:col[2])
    let bg_gui = s:checkargs(s:col[3])
    
    " Build the highlight command
    let cmd = "hi " . s:col[0]
    let cmd .= " cterm=" . s:col[1]
    let cmd .= " gui=" . s:col[1]
    let cmd .= " ctermfg=" . fg_term
    let cmd .= " ctermbg=" . bg_term
...
" vim: fdm=marker ft=vim ff=unix sw=4 ts=4 sts=4 et
" End of file.
