...
- Mutable, not sourced from the repo: `config.toml`, `auth.json`, sqlite state, `history.jsonl`, `sessions/`, caches. The base `config.toml` deliberately carries CLI-wide opt-outs of the OpenAI-bundled surface (2026-07): all six `@openai-bundled` plugins disabled (`browser`, `chrome`, `computer-use`, `sites`, `visualize`, `deep-research`), the five `.system` skills disabled via `[[skills.config]]` name selectors, and `features.remote_plugin = false` (kills the `<recommended_plugins>` account-connector suggestions such as Google Drive — those are ChatGPT account-level connectors, not local files, so directory isolation alone cannot hide them). The `openai` profile opts back in (`remote_plugin = true` plus explicit skill enables). Preserve these stanzas when touching `~/.codex/config.toml`.
