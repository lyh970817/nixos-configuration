...

{
  ...
  # URL whose scheme has no handler -- `about:blank`, which some OAuth login
  # flows probe with before they open the real auth URL -- xdg-open falls back
  # to a hardcoded list that starts `x-www-browser:firefox:...`, and firefox is
  # what answers. $BROWSER is consulted before that list, so declaring it here is what
