...

{
  ...
  # never creates the login keyring on its own. Without the file the first
  # consumer (Claude Desktop, Brave/Chromium Safe Storage, Bitwarden) only finds
  # the volatile `session` collection and gcr pops its "choose a password for
  # the new keyring" prompt — exactly the interaction that must not happen on a
