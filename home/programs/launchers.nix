...
let
  ...
  # NoDisplay stub at the same relative path, force = true so it wins over the
  # package's own entry (~/.local/share outranks the profile) -> single hidden
  # entry, no duplicate. `path` is relative to applications/ and may include
  # subdirectories (e.g. Wine's Programs/<app>/<entry>); Name is derived from
  # its last segment.
  hideFromLauncher = path: {
    name = "applications/${path}.desktop";
    value = {
      force = true;
      text = ''
        [Desktop Entry]
        Type=Application
        Name=${pkgs.lib.lists.last (pkgs.lib.strings.splitString "/" path)}
        NoDisplay=true
      '';
    };
  };
  ...
  # The Mplus Wine installer regenerates these Start-menu entries under
  # ~/.local/share/applications/wine/Programs/Mplus/ on every reinstall, and
  # rofi's drun mode scans applications/ recursively so they surface in the
  # launcher from that subdirectory too. Hiding them declaratively (rather
  # than hand-cleaning the directory, as was done on the home machine) is what
  # keeps both machines in agreement.
  wineMplusStartMenuEntries = [
    "wine/Programs/Mplus/Mplus Editor"
    "wine/Programs/Mplus/Mplus Examples"
    "wine/Programs/Mplus/Documentation"
    "wine/Programs/Mplus/Mplus Help"
    "wine/Programs/Mplus/Uninstall Mplus"
  ];

in
{
  xdg.dataFile =
    builtins.listToAttrs (map hideFromLauncher (hiddenLauncherEntries ++ wineMplusStartMenuEntries))
    // {
      ...
      "applications/yazi.desktop" = {
        ...
        text = ''
          ...
          # provide the standard system-file-manager icon name.
